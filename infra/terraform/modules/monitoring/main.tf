...
resource "aws_cloudwatch_metric_alarm" "alb_target_5xx_errors" {
  alarm_name          = "${var.name_prefix}-target-5xx-errors"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 2
  ...
  statistic           = "Sum"
  threshold           = var.alb_5xx_threshold
  alarm_description   = "Target 5xx error rate is high"
  alarm_actions       = [aws_sns_topic.alarms.arn]
  ok_actions          = [aws_sns_topic.alarms.arn]
  treat_missing_data  = "notBreaching"
...
resource "aws_cloudwatch_metric_alarm" "rds_serverless_capacity" {
  alarm_name          = "${var.name_prefix}-rds-capacity-high"
  comparison_operator = "GreaterThanThreshold"
  ...
  period              = 300
  statistic           = "Average"
  threshold           = 8 # Alert when consistently scaling above 8 ACUs
  alarm_description   = "RDS Serverless capacity is scaling high"
  alarm_actions       = [aws_sns_topic.alarms.arn]
  ok_actions          = [aws_sns_topic.alarms.arn]
  treat_missing_data  = "notBreaching"
